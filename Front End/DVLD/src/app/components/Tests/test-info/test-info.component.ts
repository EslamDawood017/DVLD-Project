...
export class TestInfoComponent implements OnInit {
  ...
  getPersonId() {
    this.localDrivingApplicationService
      .getPersonIdForLocalDrivingLicenseApp(
        this.localDrivingLicenseApp.localDrivingLicenseApplicationID,
      )
      .subscribe({
        next: (res) => {
          this.PersonId = res;
  ...
  getPassedTest() {
    this.TestService.GetNumberOfPassedTest(
      this.localDrivingLicenseApp.localDrivingLicenseApplicationID,
    ).subscribe({
      next: (res) => {
        this.PassedTest = res;
  ...
  getAllTestAppointment() {
    this.TestAppointmentService.getAllTestAppointmentPerTestType(
      this.localDrivingLicenseApp.localDrivingLicenseApplicationID,
      this.TestTypeId,
    ).subscribe({
      next: (res) => {
  ...
  AddNew() {
    if (this.allTestAppointment.length == 0) {
      this.router.navigate(['/ScheduleTest', this.TestTypeId], {
        state: { appData: this.localDrivingLicenseApp },
      });
      return;
    }

    this.TestService.GetLastTestByPersonAndTestTypeAndLicenseClass(
      this.PersonId,
      this.LicenseClassId,
      this.TestTypeId,
    ).subscribe({
  ...
  UpdateAppointmentDate() {
    this.router.navigate(['/UpdateTestAppointmentDate', this.TestTypeId], {
      state: { appData: this.localDrivingLicenseApp },
    });
  }
  TakeTest(app: any) {
    ...
    this.router.navigate(['/TakeTest', this.TestTypeId], {
      state: { appData: data, NavigationData: this.localDrivingLicenseApp },
    });
  }
