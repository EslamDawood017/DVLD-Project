...
export class GetAllLocalDrivingLicenseAppComponent implements OnInit {
  ...
  constructor(
    private LocalLicenseAppService: LocalDrivingLicenseApplicationService,
  ...
  VissionTestApplication(app: LocalDrivingLicenseApplication) {
    this.router.navigate(['/TestInfo', enTestType.VisionTest], {
      state: { appData: app },
    });
  }
  ...
  WrittenTestApplication(app: LocalDrivingLicenseApplication) {
    this.router.navigate(['/TestInfo', enTestType.WrittenTest], {
      state: { appData: app },
    });
  }
  PracticalTestApplication(app: LocalDrivingLicenseApplication) {
    this.router.navigate(['/TestInfo', enTestType.PracticalTest], {
      state: { appData: app },
    });
  }
  CancelApplicationStatus(app: any) {
    console.log('Cancel Application:', app);
    this.LocalLicenseAppService.UpdateLocalDrivingLicenseApplication(
      app.localDrivingLicenseApplicationID,
      enStatus.Cancelled,
    ).subscribe({
      next: () => {
  ...
  viewDetails(app: any) {
    this.router.navigate(['/LocalDrivingLicenseApplicationDetails'], {
      state: { appData: app },
    });
  }
  ShowLicenseInfoHistory(
    LocalDrivingLicenseApplication: LocalDrivingLicenseApplication,
  ) {
    this.router.navigate([
      '/licenseHistory',
      LocalDrivingLicenseApplication.nationalNo,
    ]);
  }
  ...
  getLicenseExist(
    ...
    this.LicenseService.IsLicenseExistByLocalDrivingLicenseApplicationID(
      localDrivingLicenseApplicationID,
      LicenseClassName,
    ).subscribe({
      next: (res) => {
